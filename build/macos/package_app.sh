...
ARCHIVE_PATH="$BUILD_ROOT/archives/SyncClipboard.xcarchive"
DIST_DIR="${DIST_DIR:-$ROOT_DIR/dist}"
APP_BUNDLE="$DIST_DIR/SyncClipboard.app"
APP_CONTENTS="$APP_BUNDLE/Contents"
APP_MACOS="$APP_CONTENTS/MacOS"
APP_RESOURCES="$APP_CONTENTS/Resources"
TEMPLATE_PLIST="$ROOT_DIR/build/macos/Info.plist.template"
ICON_SOURCE="$ROOT_DIR/build/macos/icon.icns"
ARCHIVED_BINARY="$ARCHIVE_PATH/Products/usr/local/bin/SyncClipboard"
MARKETING_VERSION="${MARKETING_VERSION:-0.1.0}"
BUILD_VERSION="${BUILD_VERSION:-$(date +%Y%m%d%H%M%S)}"
ZIP_PATH="$DIST_DIR/SyncClipboard-${MARKETING_VERSION}-macOS.zip"

rm -rf "$ARCHIVE_PATH" "$APP_BUNDLE" "$ZIP_PATH"
mkdir -p "$DIST_DIR" "$APP_MACOS" "$APP_RESOURCES"

xcodebuild \
  -scheme SyncClipboard \
...
cp "$ARCHIVED_BINARY" "$APP_MACOS/SyncClipboard"
chmod +x "$APP_MACOS/SyncClipboard"
